...
var calendarHeatmap = {
  ...
  createElements: function() {
    ...
    var calcDimensions = function() {
	  ...
	  calendarHeatmap.settings.item_size = ((calendarHeatmap.settings.width - calendarHeatmap.settings.label_padding) / numWeeks - calendarHeatmap.settings.gutter) < 20 ? 25 : ((calendarHeatmap.settings.width - calendarHeatmap.settings.label_padding) / numWeeks - calendarHeatmap.settings.gutter);
      calendarHeatmap.settings.height = calendarHeatmap.settings.label_padding + 7 * (calendarHeatmap.settings.item_size + calendarHeatmap.settings.gutter);
  ...
  parseServerData: function(serverData) {	 
	...
	for (var d in objData) {
		...
		result.push({
			...
			'details':arrDetails ,
			'total': total	,
			'strProjects':arrDetails.length == 1 ? arrDetails[0].strProjects : strProjects,
  ...
  drawGlobalOverview: function() {
    ...
    var item_block = calendarHeatmap.items.selectAll('.item-block-year')
      ...
      .on('mouseover', function(d) {
        ...
        var y = Math.abs((topY - document.getElementById(calendarHeatmap.container).getBoundingClientRect().height)) + calendarHeatmap.settings.tooltip_padding;
	...
	/****************************/
	  ...
	  .text(function(d, i) {
		return moment(d3.select(this.parentNode).attr('date')).format('DD/MM') 
		 + ' : '  + 		(d3.select(this.parentNode).attr('total') ? calendarHeatmap.formatTime(d3.select(this.parentNode).attr('total')) : 'No time');
	  }) 
	  .style('fill',function(d,i) {		
...
/****************************/
  ...
  drawDayOverview: function() {
    ...
    calendarHeatmap.labels.selectAll('.label-project')
      ...
      .attr('y', function(d) {
		  lastY = lastY + projectScale(d) + projectScale.bandwidth() / 2;
